...
class deflector(gen_lens):
    ...
    def potential(self):
        ...
        x0 = y0 = (potential.shape[0]) / 2.0*self.pixel_scale-self.size/2.0
        ...
        print (x.min(),x.max(),y.min(),y.max(),x0,y0)
        print ('-',x_.min(),x_.max(),y_.min(),y_.max())
        print (pot[-1,:].min(),pot[-1,:].max(),pot[:,-1].min(),pot[:,-1].max())
    ...
    def mapCrop(self,mappa):
        ...
        print (xmin,xmax,mappa.shape,self.nx,self.ny)
        return(mappa)
